...
func NewProtonDrive(ctx context.Context, config *common.Config, authHandler proton.AuthHandler, deAuthHandler proton.Handler) (*ProtonDrive, *common.ProtonDriveCredential, error) {
	...
	*/
	...
	log.Printf("all volumes: %d", len(volumes))

	// Get all shares first so we can find the correct primary main share,
	// even if multiple volumes are active (e.g. Files + Photos volumes).
	allShares, err := getAllShares(ctx, c)
	if err != nil {
		return nil, nil, err
	}

	// Find the main share: prefer a share explicitly flagged PrimaryShare+Main.
	// Fall back to any share from an active volume to handle accounts where
	// the API returns flags as a bitmask (Flags & PrimaryShare != 0) rather
	// than the exact constant (Flags == PrimaryShare).
	mainShareID := ""
	for i := range allShares {
		if allShares[i].Flags&proton.PrimaryShare != 0 &&
			allShares[i].Type == proton.ShareTypeMain {
			mainShareID = allShares[i].ShareID
			break
		}
	}
	if mainShareID == "" {
		// Fallback: first active volume's share (original behaviour).
		for i := range volumes {
	...
	// Verify the selected share is indeed the primary main share.
	// Use bitmask check for Flags in case the API returns combined flag values.
	{
		mainShareCheck := false
		for i := range allShares {
			if allShares[i].ShareID == mainShare.ShareID &&
				allShares[i].LinkID == mainShare.LinkID &&
				allShares[i].Flags&proton.PrimaryShare != 0 &&
				allShares[i].Type == proton.ShareTypeMain {
				mainShareCheck = true
			}
		}
		...
		if !mainShareCheck {
			...
			log.Printf("allShares %#v", allShares)
