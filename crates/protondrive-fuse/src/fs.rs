...
impl InodeMap {
    ...
    fn alloc(&mut self, n: &NodeId) -> u64 {
        if let Some(&i) = self.node_to_ino.get(n) {
            return i;
        }
        let i = self.next;
        self.next += 1;
...
impl ProtonFs {
    pub fn new(daemon: &Daemon, root: Option<NodeId>) -> Self {
        ...
        if let Some(r) = &root {
            inodes.ino_to_node.insert(ROOT_INO, r.clone());
            inodes.node_to_ino.insert(r.clone(), ROOT_INO);
        }
        Self {
            db: daemon.db.clone(),
    ...
    fn attr_for(&self, ino: u64, n: &protondrive_core::types::Node) -> FileAttr {
        ...
        FileAttr {
            ...
            crtime: mtime,
            kind,
            perm: if matches!(n.kind, NodeKind::Folder) {
                0o755
            } else {
                0o644
            },
...
impl Filesystem for ProtonFs {
    fn lookup(&mut self, _req: &Request<'_>, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        let kids = match self.db.list_children(&parent_id) {
            Ok(k) => k,
            Err(_) => {
